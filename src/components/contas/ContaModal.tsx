...
const ContaModal = ({ open, onOpenChange, onSuccess, conta, grupoId }: ContaModalProps) => {
  ...
  useEffect(() => {
    if (open) {
      if (conta) {
        // Preencher os dados da conta ao abrir para edição
        setFormData({
          con_nome: conta.con_nome || '',
          con_tipo: conta.con_tipo || 'banco',
          con_limite: (conta.con_limite || 0).toString(), // Garantir que seja string
          con_banco: conta.con_banco || ''
        });
      } else {
        // Resetar para valores padrão ao criar nova conta
        setFormData({
          con_nome: '',
          con_tipo: 'banco',
          con_limite: '0',
  ...
  }, [conta, open]); // Dependências importantes: conta e open
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        con_limite: parseFloat(formData.con_limite) || 0, // Converter para número
      ...
      onSuccess(); // Atualizar a lista de contas na página principal
      ...
      <DialogContent className="sm:max-w-[425px] bg-white border border-border-light shadow-xl p-6 rounded-3xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.con_tipo} onValueChange={val => setFormData({ ...formData, con_tipo: val })}>
