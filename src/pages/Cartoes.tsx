...
import { CreditCard, Landmark, ReceiptText, Wallet, Plus, ArrowRight, TrendingUp, DollarSign, Settings, Trash2, AlertCircle, Eye, EyeOff, Search } from 'lucide-react';
...
const Cartoes = ({ hideValues }: { hideValues: boolean }) => {
  ...
  const [creditCardAccounts, setCreditCardAccounts] = useState<CardAccount[]>([]);
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const transferenciaCat = categoriesData.data?.find((cat: any) => 
        cat.cat_nome === 'Transferência entre Contas' && cat.cat_tipo === 'sistema');
      const pagamentoFaturaCat = categoriesData.data?.find((cat: any) => 
        cat.cat_nome === 'Pagamento de Fatura' && cat.cat_tipo === 'sistema');
        ...
        .map((card: any) => {
          ...
          if (diaFechamento && diaVencimento) {
            ...
            const cardTransactions = (lancamentosData.data || []).filter(
              (lan: any) => 
                lan.lan_conta === card.con_id && 
                isAfter(parseISO(lan.lan_data), cycleStartDate) && 
                isBefore(parseISO(lan.lan_data), addMonths(cycleEndDate, 1)) && 
                !lan.lan_pagamento
            );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
          ...
          <Card className="bg-white dark:bg-[#1e1629] p-6 rounded-2xl border border-[#f2f0f4] dark:border-[#2d2438] shadow-sm flex flex-col gap-1 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[#756189] font-medium text-sm">Limite Total</span>
            </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {creditCardAccounts.length === 0 ? (
              ...
              creditCardAccounts
                .filter(card => 
                  card.con_nome.toLowerCase().includes(searchTerm.toLowerCase()) ||
                  card.con_id.toLowerCase().includes(searchTerm.toLowerCase())
                )
                .map((card) => {
                  ...
                  return (
                    <Card 
                      key={card.con_id} 
                      className="bg-white dark:bg-[#1e1629] rounded-2xl p-6 shadow-sm border border-[#f2f0f4] dark:border-[#2d2438] hover:shadow-md transition-all group flex flex-col justify-between h-full relative overflow-hidden"
                    >
                        ...
                        <div className="flex items-start justify-between mb-6 relative z-10">
                          <div className="flex items-center gap-4">
                            <div 
                              ...
                              style={{ 
                                backgroundColor: card.con_banco === 'Nubank' ? '#820AD1' : 
                                                card.con_banco === 'XP' ? '#141118' : 
                                                card.con_banco === 'Inter' ? '#FF7A00' : 
                                                '#7311d4' 
                              }}
                            >
                          ...
                          <div className="flex flex-col items-end gap-1">
                            <span className="px-2.5 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 text-xs font-semibold text-[#756189]">Física</span>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-[#f2f0f4] dark:border-[#2d2438] relative z-10">
                        <div className="flex gap-6">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className={cn(
                              "text-sm font-bold text-[#141118] dark:text-white flex items-center gap-1",
                              card.faturaStatus === 'vencida' && 'text-red-600'
                            )}>
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <Button 
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleOpenDeleteConfirm(card.con_id, card.con_nome);
                            }}
                          >
